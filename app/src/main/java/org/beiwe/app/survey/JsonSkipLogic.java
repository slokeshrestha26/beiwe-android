...
public class JsonSkipLogic {
	...
	private HashMap<String, Boolean> QuestionRequired;
	...
	public JsonSkipLogic (JSONArray jsonQuestions, Boolean runDisplayLogic, Context applicationContext) throws JSONException {
		...
		QuestionRequired = new HashMap<String, Boolean>(MAX_SIZE);
		...
		for (int i = 0; i < MAX_SIZE; i++) { //uhg, you can't iterate over a JSONArray.
			...
			QuestionRequired.put(questionId, requiredQuestion); //setup required questions
	...
	public QuestionData getQuestionAnswer (String questionId) {
	...
	public Boolean getCurrentQuentionRequired() {
		if (currentQuestion >= QuestionOrder.size() || currentQuestion < 0) {
			return null;
		}
		return QuestionRequired.get(QuestionOrder.get(currentQuestion));
	}
...
//			Log.d("json logic", "Question " + QuestionOrder.indexOf(questionId) + " (" + questionId + ") did not evaluate as true, proceeding to next question...");
	...
	public JSONObject goBackOneQuestion () {
...
//		Log.d("logic...", "evaluating useranswer " + userAnswer + comparator + surveyValue);
	...
	// FIXME: getAnswerDouble is literally only used internal to this function, which means it simply shouldn't exist..
	...
	public void setAnswer (QuestionData questionData) {
		...
		if (questionType.equals(QuestionType.Type.SLIDER)) {
	...
	public Boolean hasAnswer (String questionId) {
		return QuestionAnswer.containsKey(questionId);
	}
